// src/components/admin/homepage/TestimonialsForm.tsx
'use client';
...
const TestimonialsForm = memo(function TestimonialsForm({ sectionKey, initialData }: TestimonialsFormProps) {
  ...
  const addTestimonial = useCallback(() => {
    setFormData((prev: any) => ({
      ...prev,
      testimonials: [...(prev.testimonials || []), { text: '', name: '', image_url: '' }],
    }));
  }, []);
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm space-y-6 max-w-4xl">
      <div className="border-b border-gray-200 pb-4">
        <h3 className="text-lg font-semibold text-orange-600">Testimonials Section</h3>
        <p className="text-sm text-gray-500 mt-1">Edit testimonials content</p>
      </div>
        ...
        {(formData.testimonials || []).map((testimonial: any, index: number) => (
          <div key={index} className="border rounded-lg p-4 mb-3 bg-gray-50">
            <div className="flex justify-between items-start mb-2">
              ...
              <button type="button" onClick={() => removeTestimonial(index)} className="p-1.5 text-red-500 hover:text-red-700"><Trash2 size={16} /></button>
            </div>
            <div className="grid grid-cols-1 gap-3">
              <div>
                <label className="block text-xs font-medium mb-1">Text</label>
                <textarea value={testimonial.text || ''} onChange={(e) => handleTestimonialChange(index, 'text', e.target.value)} rows={3} className="w-full border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1">Name</label>
                <input type="text" value={testimonial.name || ''} onChange={(e) => handleTestimonialChange(index, 'name', e.target.value)} className="w-full border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1">Image URL</label>
                <input type="text" value={testimonial.image_url || ''} onChange={(e) => handleTestimonialChange(index, 'image_url', e.target.value)} className="w-full border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none" placeholder="/images/path/to/image.jpg" />
              </div>
            </div>
          </div>
        ))}
        <button type="button" onClick={addTestimonial} className="flex items-center gap-2 text-orange-600 hover:text-orange-700 text-sm font-medium"><Plus size={16} /> Add Testimonial</button>
      </div>
...
TestimonialsForm.displayName = 'TestimonialsForm';
