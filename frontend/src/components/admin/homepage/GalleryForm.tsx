// src/components/admin/homepage/GalleryForm.tsx
'use client';
...
const GalleryForm = memo(function GalleryForm({ sectionKey, initialData }: GalleryFormProps) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Gallery section updated successfully! 🎉');
      router.refresh();
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm space-y-6 max-w-4xl">
      <div className="border-b border-gray-200 pb-4">
        <h3 className="text-lg font-semibold text-orange-600">Gallery Section</h3>
        <p className="text-sm text-gray-500 mt-1">Edit gallery content</p>
      </div>
      ...
      <div>
        ...
        {(formData.images || []).map((image: any, index: number) => (
          <div key={index} className="flex gap-2 mb-2">
            <input type="text" value={image.src || ''} onChange={(e) => handleImageChange(index, 'src', e.target.value)} className="flex-1 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none" placeholder="Image URL" />
            <input type="text" value={image.title || ''} onChange={(e) => handleImageChange(index, 'title', e.target.value)} className="flex-1 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none" placeholder="Image Title" />
            <button type="button" onClick={() => removeImage(index)} className="p-2 text-red-500 hover:text-red-700"><Trash2 size={18} /></button>
          </div>
        ))}
        <button type="button" onClick={addImage} className="flex items-center gap-2 text-orange-600 hover:text-orange-700 text-sm font-medium"><Plus size={16} /> Add Image</button>
      </div>
