// src/components/admin/homepage/MissionForm.tsx
'use client';
...
const MissionForm = memo(function MissionForm({ sectionKey, initialData }: MissionFormProps) {
  ...
  const handleArrayChange = useCallback((index: number, value: string) => {
  ...
  const handleImageUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError('');
    try {
      const url = await uploadHomeImage(file, 'mission');
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess('Mission section updated successfully! 🎉');
      router.refresh();
      setTimeout(() => setSuccess(''), 3000);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-sm space-y-6 max-w-4xl">
      <div className="border-b border-gray-200 pb-4">
        <h3 className="text-lg font-semibold text-orange-600">Mission Section</h3>
        <p className="text-sm text-gray-500 mt-1">Edit mission section content</p>
      </div>
      ...
      <div>
        ...
        {(formData.points || []).map((point: string, index: number) => (
          <div key={index} className="flex gap-2 mb-2">
            <input type="text" value={point} onChange={(e) => handleArrayChange(index, e.target.value)} className="flex-1 border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none" placeholder={`Point ${index + 1}`} />
            <button type="button" onClick={() => removePoint(index)} className="p-2 text-red-500 hover:text-red-700"><Trash2 size={18} /></button>
          </div>
        ))}
        <button type="button" onClick={addPoint} className="flex items-center gap-2 text-orange-600 hover:text-orange-700 text-sm font-medium"><Plus size={16} /> Add Point</button>
      </div>
