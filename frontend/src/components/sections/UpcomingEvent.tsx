...
// src/components/sections/UpcomingEvent.tsx
'use client';
...
export default function UpcomingEvents({ data }: UpcomingEventProps) {
  const title = data?.title || 'Upcoming Events';
  const subtitle = data?.subtitle || 'INVOLVE NOW';
  const description = data?.description || 'Join our transformative events and be part of positive change';

  const featuredEvent = data?.featured_event || defaultFeaturedEvent;
  const regularEvents = data?.events?.length ? data.events : defaultEvents;
    ...
    <section className="py-20 bg-white dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mt-2 mb-4">{title}</h2>
          <div className="w-20 h-1 bg-gradient-to-r from-orange-500 to-pink-500 mx-auto" />
        ...
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5">
            ...
            >
              <div className="relative h-[500px] w-full">
                <OptimizedImage
                  src={featuredEvent.image_url || '/images/events/1.jpg'}
                  alt={featuredEvent.title}
                  sizes="160px"
                  className="object-cover group-hover:scale-110 transition-transform duration-700"
                />
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 text-white z-10">
                ...
                <p className="text-white/80 text-sm mb-4 line-clamp-2">{featuredEvent.description}</p>
                <div className="flex gap-3">
                  <Link
                    href="/donate"
                    className="px-5 py-2 bg-gradient-to-r from-orange-500 to-pink-500 rounded-lg text-sm font-semibold hover:shadow-lg transition-all"
