...
const PartnerLogo = memo(({ logo, name }: { logo: string; name: string }) => (
  <div className="flex items-center justify-center p-4 bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl hover:scale-105 transition-all duration-300 mx-3 flex-shrink-0">
    <div className="relative w-24 h-16">
      <OptimizedImage src={logo} fill sizes="160px" alt={name} className="object-contain" />
    </div>
  </div>
));
...
export default function PartnersSection({ data }: PartnersSectionProps) {
  const title = data?.title || 'Our Trusted Partners';
  const subtitle = data?.subtitle || 'Working together for a better world';
