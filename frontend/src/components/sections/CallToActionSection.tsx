...
export default function CallToActionSection({ data, setVideoModalOpen }: CallToActionSectionProps) {
  const title = data?.title || 'Fundraising for the people and causes you care about';
  const subtitle = data?.subtitle || 'Call to Action';
  const description =
    data?.description ||
    'LPM promotes the philosophy that life is not based on luck or coincidence, but on conscious construction through positive thinking, discipline, and spiritual awakening.';
  const imageUrl = data?.image_url || '/images/home/fundrising.jpg';
  const buttonText = data?.button_text || 'PLAY SHORT VIDEO';

  return (
    <section className="relative py-24 overflow-hidden">
      <div className="absolute inset-0">
        <OptimizedImage src={imageUrl} alt="CTA" sizes="100vw" priority className="object-cover" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/80 to-black/60" />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial="hidden" whileInView="visible" variants={slideInLeft}>
            <span className="text-orange-300 font-semibold tracking-wide uppercase">{subtitle}</span>
