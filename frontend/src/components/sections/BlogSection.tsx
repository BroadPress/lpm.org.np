...
const defaultPosts: BlogPost[] = [
  ...
  },
  {
    title: 'Traffic Awareness Program',
    description: 'A public awareness initiative designed to educate citizens, students, and drivers about road safety, traffic discipline & responsible driving.',
    image_url: '/images/gallery/19.jpg',
    author: 'Life Positive Mission',
    date: 'September 17, 2025',
    category: 'Awareness',
  },
  {
    title: 'Entrepreneurship Award',
    description: 'A prestigious recognition program honoring innovative entrepreneurs, startup founders, and business leaders who are contributing to economic growth, employment generation, and positive social impact.',
    image_url: '/images/gallery/31.jpg',
    author: 'Life Positive Mission',
    date: 'March 17, 2026',
    category: 'Award',
  },
...
const BlogPostComponent = ({ post, index }: { post: BlogPost; index: number }) => (
  <motion.article
    initial={{ opacity: 0, y: 30 }}
    whileInView={{ opacity: 1, y: 0 }}
    transition={{ delay: index * 0.1 }}
...
export default function BlogSection({ data }: BlogSectionProps) {
  const title = data?.title || 'News & Updates';
  const subtitle = data?.subtitle || 'Latest Updates';
  const posts = data?.posts?.length ? data.posts : defaultPosts;
  ...
  return (
    <section className="py-20 bg-white dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {posts.map((post, idx) => (
            <BlogPostComponent key={idx} post={post} index={idx} />
          ))}
        </div>
      </div>
    </section>
